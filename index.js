...
app.get('/', (req, res) => res.send('ZUZ Universe Bot is running ✅'));
...
bot.hears('💰 Мой баланс', async (ctx) => {
    const db = getDB();
    const user = await db.get('SELECT balance_zuz FROM users WHERE user_id = ?', ctx.from.id);
...
bot.action('copy_ref', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.reply(`🔗 *Ваша ссылка:*\n\`https://t.me/zuzim_universe_bot?start=ref_${ctx.from.id}\``, { parse_mode: 'Markdown' });
});
...
bot.command('referral', async (ctx) => {
    await ctx.reply(`🔗 \`https://t.me/zuzim_universe_bot?start=ref_${ctx.from.id}\``, { parse_mode: 'Markdown' });
});
...
async function start() {
    await initDB();
    await bot.launch();
    console.log('🚀 Бот ZUZ Universe запущен и готов принимать сообщения!');
